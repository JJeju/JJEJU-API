...
public class PagingUtil {   
      private int maxNum; //전체 글 개수 저장하는 변수
      private int pageNum; //현재 보이는 페이지의 번호 저장 변수
      private int listCnt; //한 페이지당 보이는 게시글의 개수 저장 변수
      private int pageCnt; //보여질 페이지 번호 개수 저장 변수
      private String listName; //다수의 게시판이 있을 경우 해당 게시판의 url을 저장
      ...
      public String makePaging() {
         ...
         //1. 전체 페이지 개수 구하기(페이지 당 listCnt(10) 만큼 출력)
         //전체 게시글이 9개 : 1개 
         //전체 게시글이 11개 : 2개
         ...
         if(start != 1) {
            ...
            sb.append("&nbsp;이전&nbsp;</a>");
         } // <a class='pno' herf='./?list?pageNum=5'> dlwjs </a>
         //페이지 번호를 5개씩 보여주는 경우
         //6페이지 보이는 화면에서 이전 버튼이 보이게 되고
